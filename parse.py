...
fig.add_trace(go.Scatter(
    ...
    y=df['f1_score'],
    mode='lines+markers',
    text=df.apply(lambda row: f'Order ID:{row["order_id"]}<br>Positive count:{row["true_positives_count"]}<br>False positive count:{row["false_positives_count"]}<br>False negative count:{row["false_positives_count"]}', axis=1),
    hoverinfo='text',
))
